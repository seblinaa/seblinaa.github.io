...
## Review Questions from Module One Handbook?
 - what can we `typeof` ? dataTypes except the below
 - what can't we `typeof` ? arrays, objects, null and date
 - how do we check those things? using if statements
 - Array.isArray()
 - null === null

## Standard Library Methods You Should Know By Now:
 - Object.keys() access the keys in an object
 - String.toUpperCase() uppercase the given string
 - String.split() separate the string
 - Array.join() join the array to a string
